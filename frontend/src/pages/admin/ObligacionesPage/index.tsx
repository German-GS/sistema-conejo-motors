...
const notaInp: React.CSSProperties = { display: "block", width: "100%", marginTop: 3, padding: "0.4rem 0.5rem", borderRadius: 8, border: "1.5px solid var(--slate-200)", fontSize: "0.82rem", fontFamily: "inherit", boxSizing: "border-box" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        <div style={card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.5rem", marginBottom: "1rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
              ...
              <input type="month" value={periodoSel} onChange={(e) => setPeriodoSel(e.target.value)} style={{ padding: "0.35rem 0.5rem", borderRadius: 8, border: "1.5px solid var(--slate-200)", fontSize: "0.9rem" }} />
              <span style={{ fontSize: "0.72rem", color: "var(--slate-400)" }}>(podés elegir cualquier mes para reconstruir hacia atrás)</span>
            </div>
            <span style={{ fontSize: "0.78rem", color: "var(--slate-500)" }}>Prorrata: <strong>{calc.porcentaje_prorrata}%</strong></span>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "0.75rem", marginBottom: "1rem" }}>
            {[
              ["Débito fiscal (ventas)", calc.debito_fiscal, "var(--danger)"],
              ["Crédito fiscal bruto", calc.credito_fiscal_bruto, "var(--info)"],
              ["Crédito aplicable (prorrata)", calc.credito_fiscal_aplicable, "var(--success)"],
              ["IVA a pagar", calc.iva_a_pagar, calc.iva_a_pagar > 0 ? "var(--danger)" : "var(--success)"],
            ].map(([l, v, c]) => (
              <div key={l as string} style={{ border: "1px solid var(--slate-100)", borderRadius: 10, padding: "0.7rem 0.9rem" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {Object.entries(calc.ventas_por_tarifa ?? {}).map(([k, v]: any) => (
                    <tr key={k} style={{ borderTop: "1px solid var(--slate-100)" }}>
          ...
          {/* Retenciones + generar */}
          <div style={{ display: "flex", alignItems: "flex-end", gap: "1rem", flexWrap: "wrap", marginTop: "1rem", borderTop: "1px solid var(--slate-100)", paddingTop: "1rem" }}>
            <label style={{ fontSize: "0.8rem", fontWeight: 600, color: "var(--slate-600)" }}>Retenciones IVA (datáfono/tarjeta)
              <input type="number" value={retenciones} onChange={(e) => setRetenciones(Number(e.target.value))} style={{ display: "block", marginTop: 4, padding: "0.45rem 0.6rem", borderRadius: 8, border: "1.5px solid var(--slate-200)", width: 160 }} />
            </label>
      ...
      <div style={card}>
        ...
        <p style={{ fontSize: "0.8rem", color: "var(--slate-500)", margin: "0.25rem 0 0.75rem" }}>Devoluciones, descuentos o cargos posteriores que ajustan el IVA del período (crédito resta, débito suma).</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "0.6rem", alignItems: "end" }}>
          <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--slate-600)" }}>Tipo
          ...
          <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--slate-600)" }}>Naturaleza
          ...
          <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--slate-600)" }}>Tarifa
          ...
          <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--slate-600)" }}>Base (₡)
            <input type="number" value={nota.base} onChange={(e) => setNota({ ...nota, base: e.target.value })} style={notaInp} />
          </label>
          <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--slate-600)" }}>IVA (auto)
            <input type="number" value={nota.iva} onChange={(e) => setNota({ ...nota, iva: e.target.value })} placeholder="auto" style={notaInp} />
          </label>
          <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--slate-600)" }}>Doc. ref
      ...
      <div style={{ ...card, background: "var(--slate-50)" }}>
        ...
        <ul style={{ fontSize: "0.84rem", color: "var(--slate-600)", lineHeight: 1.7, marginTop: "0.5rem" }}>
