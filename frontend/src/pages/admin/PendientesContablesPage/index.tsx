...
const th: React.CSSProperties = { padding: "8px 10px", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.03em", color: "var(--slate-500)", textAlign: "left" };
const td: React.CSSProperties = { padding: "8px 10px", color: "var(--slate-700)", fontSize: "0.85rem", verticalAlign: "top" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.75rem" }}>
        ...
        <button onClick={cargar} style={{ background: "#fff", border: "1.5px solid var(--slate-200)", color: "var(--slate-600)", borderRadius: 8, padding: "0.5rem 1rem", cursor: "pointer", fontWeight: 600, display: "flex", alignItems: "center", gap: "0.4rem" }}><LuRefreshCw size={15} /> Actualizar</button>
      </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 640 }}>
              ...
              <tbody>
                {data.documentos.map((d, i) => (
                  <tr key={`${d.origen}-${d.id}-${i}`} style={{ borderTop: "1px solid var(--slate-100)" }}>
