...
const th: React.CSSProperties = { padding: "6px 10px", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.03em", color: "var(--slate-500)", textAlign: "left" };
...
const Delta = ({ actual, anterior, favorableCuandoSube = true }: { actual: number; anterior?: number; favorableCuandoSube?: boolean }) => {
  if (anterior === undefined || anterior === null || anterior === "" as any) return <span style={{ color: "var(--slate-400)" }}>—</span>;
  const d = +(actual - anterior).toFixed(2);
  if (Math.abs(d) < 0.01) return <span style={{ color: "var(--slate-400)" }}>=</span>;
  const sube = d > 0;
  const favorable = favorableCuandoSube ? sube : !sube;
  return <span style={{ color: favorable ? "var(--success)" : "var(--danger)", fontWeight: 600 }}>{sube ? "▲" : "▼"} {CRC(Math.abs(d))}</span>;
};
...
export const EstadosFinancierosPage = () => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      <div style={{ ...card, background: "var(--slate-50)" }}>
        <p style={{ fontSize: "0.82rem", color: "var(--slate-600)", margin: 0, lineHeight: 1.6 }}>
...
const Seccion = ({ titulo, filas, anterior, totalLabel, total, totalPrev, favorableCuandoSube = true }: any) => (
  <>
    ...
    {filas.length === 0 && <tr><td style={{ ...td, color: "var(--slate-400)" }} colSpan={4}>Sin movimientos.</td></tr>}
    {filas.map((f: any) => {
      const prev = anterior?.find((x: any) => x.codigo === f.codigo);
      return (
        <tr key={f.codigo} style={{ borderTop: "1px solid var(--slate-50)" }}>
    ...
    {totalLabel && (
      <tr style={{ borderTop: "2px solid var(--slate-200)" }}>
  ...
  return (
    <Tabla>
      ...
      <tr style={{ borderTop: "3px solid var(--brand-dark)" }}>
        <td style={{ ...td, fontWeight: 800, color: a.utilidadNeta >= 0 ? "var(--success)" : "var(--danger)" }}>UTILIDAD NETA</td>
        <td style={{ ...tdR, fontWeight: 800, color: a.utilidadNeta >= 0 ? "var(--success)" : "var(--danger)" }}>{CRC(a.utilidadNeta)}</td>
...
const Fila = ({ label, monto, bold, indent, color, top }: any) => (
  <tr style={{ borderTop: top ? top : "1px solid var(--slate-50)" }}>
    <td style={{ ...td, paddingLeft: indent ?? 10, fontWeight: bold ? 800 : 400, color: color ?? (bold ? "var(--brand-dark)" : "var(--slate-700)") }} colSpan={3}>{label}</td>
    <td style={{ ...tdR, fontWeight: bold ? 800 : 400, color: color ?? "var(--slate-700)" }}>{monto === undefined ? "" : CRC(monto)}</td>
  </tr>
);
    ...
    <>
      <p style={{ fontSize: "0.8rem", color: "var(--slate-500)", margin: 0 }}>
        Corte al {a.fechaCorte} · {a.equilibrado
          ? <span style={{ color: "var(--success)", fontWeight: 700, display: "inline-flex", alignItems: "center", gap: "0.3rem" }}><LuCircleCheck size={14} /> Cuadrado</span>
          : <span style={{ color: "var(--danger)", fontWeight: 700, display: "inline-flex", alignItems: "center", gap: "0.3rem" }}><LuTriangleAlert size={14} /> Descuadrado</span>}
      </p>
      <Tabla2>
        <Fila label="ACTIVOS" bold color="var(--brand)" />
        ...
        <Fila label="PASIVOS" bold color="var(--brand)" top="3px solid var(--slate-200)" />
        ...
        <Fila label="PATRIMONIO" bold color="var(--brand)" top="3px solid var(--slate-200)" />
      ...
      <div style={{ ...card, background: a.equilibrado ? "#f0fdf4" : "#fef2f2", border: `1px solid ${a.equilibrado ? "#bbf7d0" : "#fecaca"}` }}>
        ...
        <div style={{ fontSize: "0.95rem", marginTop: 4, color: "var(--slate-700)" }}>
...
const FlujoCaja = ({ data }: any) => {
  ...
  const seccion = (titulo: string, s: any) => (
    <>
      <Fila label={titulo} bold color="var(--brand)" top="3px solid var(--slate-200)" />
      {s.items.map((it: any, i: number) => <Fila key={i} label={it.concepto} monto={it.monto} indent={26} />)}
      <Fila label={`Total ${titulo.toLowerCase()}`} monto={s.total} bold indent={10} top="1px solid var(--slate-200)" />
    </>
  );
  return (
    <>
      <p style={{ fontSize: "0.8rem", color: "var(--slate-500)", margin: 0 }}>Estado de Flujo de Efectivo — método indirecto.</p>
      <Tabla2>
        ...
        <Fila label="VARIACIÓN NETA DE EFECTIVO" monto={a.variacionNeta} bold color={a.variacionNeta >= 0 ? "var(--success)" : "var(--danger)"} top="3px solid var(--brand-dark)" />
      </Tabla2>
      ...
      <div style={{ ...card, background: a.cuadra ? "#f0fdf4" : "#fef2f2", border: `1px solid ${a.cuadra ? "#bbf7d0" : "#fecaca"}` }}>
        ...
        <div style={{ fontSize: "0.85rem", marginTop: 4, color: "var(--slate-700)" }}>
...
const SEM_COLOR: Record<string, { bg: string; border: string; text: string }> = {
  ...
  na: { bg: "var(--slate-50)", border: "var(--slate-200)", text: "var(--slate-400)" },
};
...
const TendenciaIcon = ({ t }: { t: string }) => {
  if (t === "mejora") return <span style={{ color: "var(--success)", fontWeight: 700 }} title="Mejora vs. mes anterior">▲</span>;
  if (t === "empeora") return <span style={{ color: "var(--danger)", fontWeight: 700 }} title="Empeora vs. mes anterior">▼</span>;
  return <span style={{ color: "var(--slate-400)" }} title="Estable">=</span>;
};
        ...
        {(d.fortalezas.length > 0 || d.riesgos.length > 0) && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1rem", marginTop: "1rem" }}>
            <div>
              ...
              {d.fortalezas.length === 0 ? <div style={{ fontSize: "0.8rem", color: "var(--slate-400)" }}>—</div> :
                <ul style={{ margin: 0, paddingLeft: 18, fontSize: "0.8rem", color: "var(--slate-700)", lineHeight: 1.5 }}>{d.fortalezas.map((s: string, i: number) => <li key={i}>{s}</li>)}</ul>}
            </div>
            <div>
              ...
              {d.riesgos.length === 0 ? <div style={{ fontSize: "0.8rem", color: "var(--slate-400)" }}>—</div> :
                <ul style={{ margin: 0, paddingLeft: 18, fontSize: "0.8rem", color: "var(--slate-700)", lineHeight: 1.5 }}>{d.riesgos.map((s: string, i: number) => <li key={i}>{s}</li>)}</ul>}
            </div>
          </div>
        )}
      </div>
              ...
              {items.map((i: any) => {
                ...
                return (
                  <div key={i.nombre} title={`${i.formula}\n${i.interpretacion}\nReferencia: ${i.referencia}`}
                    ...
                    <div style={{ fontSize: "0.78rem", color: "var(--slate-500)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ fontSize: "0.72rem", color: "var(--slate-400)", marginTop: 2 }}>{i.anterior != null ? `Ant: ${fmtValor(i.anterior, i.unidad)}` : " "}</div>
                    <div style={{ fontSize: "0.72rem", color: "var(--slate-500)", marginTop: 4, lineHeight: 1.35 }}>{i.interpretacion}</div>
                  </div>
                );
              })}
            </div>
